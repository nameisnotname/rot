...
SEIZURE_BANNER_PATTERNS = [
    ...
    r"seizure notice",
    # Bare "<title>Seized by X</title>" with no self-referential clause at
    # all (e.g. just "Seized by the Federal Bureau of Investigation") - a
    # real miss found by manually opening real .onion/clearnet targets and
    # diffing the monitor's CSV output against what was actually seen in
    # the browser (kratools.com: content_state stayed "normal" because none
    # of the patterns above require a self-ref phrase, and this banner
    # variant has none - only content_classifier.py's secondary signal
    # caught it, at 0.931 confidence). Deliberately scoped to the HTML
    # <title> tag specifically, not free text: a hard-negative forum post
    # discussing another site's seizure talks ABOUT a seizure, it doesn't
    # set its own page's <title> to a bare "Seized by <agency>" - so this
    # stays safe against the HARD_NEGATIVES false-positive risk that
    # motivated requiring _SELF_REF in the first place (see evaluate_
    # content_signals.py).
    r"<title>\s*seized by (?:the )?(?:fbi|federal bureau of investigation|europol|"
    r"eurojust|homeland security investigations|drug enforcement administration|"
    r"national crime agency|bundeskriminalamt|federal criminal police office)",
]
...
# monitor.py bounds every fetch to MAX_CONTENT_BYTES, so on a large page the
# read can end mid-<script>/<style> block - no closing tag is left in the
# truncated text for _SCRIPT_STYLE_RE to match, so that block's raw JS/CSS
# leaks straight through as if it were visible page text. Reproduced on a
# real captured page (TORMARKET, a Next.js SPA): its dead React
# "error"/"notFound" boilerplate JSON, cut off mid-<script>, leaked the
# literal words "error" and "404...could not be found" into
# content_classifier.py's input, producing a confident but content-free
# error_maintenance guess for a normal, fully up marketplace page. Any
# UNCLOSED <script>/<style> opening tag - by definition, one the pair regex
# above couldn't match - means everything after it to the end of the
# (already-truncated) text is presumably code, not content, so this drops
# it entirely rather than feeding it to the classifier as prose.
_DANGLING_SCRIPT_STYLE_RE = re.compile(r"<(?:script|style)\b[^>]*>.*$", re.DOTALL | re.IGNORECASE)
...
def strip_html_to_text(html):
    ...
    content), including a trailing block left unclosed by truncation,
    then strips all remaining tags (which removes their attributes too,
    since an attribute lives inside the tag's own angle brackets), then
    collapses whitespace."""
    if not html:
        return ""
    no_script_style = _SCRIPT_STYLE_RE.sub(" ", html)
    no_dangling = _DANGLING_SCRIPT_STYLE_RE.sub(" ", no_script_style)
    no_tags = _TAG_RE.sub(" ", no_dangling)
