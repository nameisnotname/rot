...
# Minimum observed span (hours) before attempting a read at all. Originally
# 48 - lowered so a read is attempted sooner. Honest note: this doesn't make
# a shorter history more reliable, it only changes when the tool is willing
# to guess at all. A stable_no_outage/ambiguous verdict off 24-30h of real
# data is a real result; it just carries less weight than the same verdict
# off a longer window, exactly the same way it always has above 48h too.
MIN_HOURS_FOR_READ = 24
...
def main():
    ...
    for name in site_names:
        ...
        span_hrs = len(hourly)
        if span_hrs < MIN_HOURS_FOR_READ:
